...
let album_reviews = {"late registration": "late_registration.html", 
"yeezus": "./todo.html", 
"aquemini": "./todo.html", 
"kids see ghosts": "./todo.html", 
"the score": "./todo.html", 
"the eminem show": "./todo.html", 
"good kid maad city": "./todo.html", 
"808s and heartbreak": "./todo.html", 
"my beautiful dark twisted fantasy": "./todo.html"};
console.log(sessionStorage.getItem("yeezus"));
for (let j = 0; j < albums.length; j++){
	...
	won[albums[j]] = (sessionStorage.getItem(albums[j]) == "true") ;
	if (won[albums[j]]){
		refresh(albums[j]);
	}
}
...
function move(k, album){
	if (won[album]){
		sessionStorage.setItem(album, true);
		document.getElementById(album).style.zIndex = 4;
		document.getElementById(album).href = album_reviews[album];
		return;
	}
	if (k % dims != dims-1){
